...
rate_list = get_best(nail_positions)
segments = [0]
lines = 0
...
while ctn:  
    changes=0
    point= rate_list.get()
    refresh_point= pixels_analysis(point[1], point[2])
    while(point[0]<refresh_point[0]):
        rate_list.put(refresh_point)
        changes+=1
        point= rate_list.get()
        refresh_point= pixels_analysis(point[1], point[2])
    bigger=refresh_point
